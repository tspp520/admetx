...
- Predictor: Python 3.12, FastAPI, uvicorn, RDKit (rdkit), pydantic
...
- **Python:** create `.venv` per project with `python3.12 -m venv .venv` (host default is 3.9; predictor needs 3.10+). Activate before running.
...
POSTGRES_PASSWORD=admetx_local
...
dependencies = [
  ...
  "rdkit>=2024.3",  # PyPI package was renamed from `rdkit-pypi` to `rdkit`
]
...
```bash
cd admetx-predictor
python3.12 -m venv .venv  # host default is 3.9
...
If the third assertion fails because the seed hasn't been applied to the dev DB, run `DATABASE_URL=postgresql://admetx:admetx_local@127.0.0.1:5436/admetx_dev pnpm db:migrate && pnpm db:seed` and rerun.
...
export async function processNext(): Promise<boolean> {
  ...
  // The `FOR UPDATE SKIP LOCKED` pattern lets multiple worker instances (future scale)
  // claim distinct tasks without racing. Even with MAX_CONCURRENCY=1 in v0.1.0, this
  // prevents the multi-task UPDATE pitfall where all queued rows flip to running in one statement.
...
# Term 2
cd admetx-web && DATABASE_URL=postgresql://admetx:admetx_local@127.0.0.1:5436/admetx_dev \
